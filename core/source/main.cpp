...
int main(int argc, char const *argv[])
{
  ...
  Object * box1 = new Object({new Transform(Vec3<float>(1, 2, 0), Vec3<float>(0.3, 0.3, 0.3), Vec3<float>(0, 0, 0), "bunny", {"initialShadingGroup"}, 0)});

  // box->addComponent(new Aabb(box->getComponent<Transform>()->getPos(), Vec3<float>(1, 1, 1), 0, "ground", box));
  // Object * ground = new Object({new Transform(Vec3<float>(0, 5.0, 0), Vec3<float>(1, 1, 1), Vec3<float>(0, 0, 0), "bunny", {"Material.001", "initialShadingGroup"}, 0)});
