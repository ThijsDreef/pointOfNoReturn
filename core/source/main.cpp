...
int main(int argc, char const *argv[])
{
  ...
  object->addComponent(new RotateTransform(Vec3<float>(0, 1.8, 0), Vec3<float>(1, 1, 1), Vec3<float>(0, 0, 0), "wizard", {"Material.001", "initialShadingGroup"}, engine.getInput(), object));
  object->addComponent(new OrbitalCamera(-8, Vec3<bool>(false, false, true), Vec3<float>(-15, -180, 0), &object->getComponent<RotateTransform>()->rotation, &object->getComponent<Transform>()->getPos(), true, object));
  ...
  Object * box1 = new Object({new Transform(Vec3<float>(1, 2, 0), Vec3<float>(0.2, 0.2, 0.2), Vec3<float>(0, 0, 0), "cube", {"initialShadingGroup"}, 0)});

  // box->addComponent(new Aabb(box->getComponent<Transform>()->getPos(), Vec3<float>(1, 1, 1), 0, "ground", box));
  Object * ground = new Object({new Transform(Vec3<float>(0, 5.0, 0), Vec3<float>(1, 1, 1), Vec3<float>(0, 0, 0), "cube", {"Material.001", "initialShadingGroup"}, 0)});
